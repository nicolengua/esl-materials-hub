...
export function buildPrompt(student, classNotes, materialTypes) {
  ...
  // Resolve native language (handle "Other" case)
  const nativeLang = student.nativeLanguage === "__other"
    ? (student.nativeLanguageOther || "Unknown")
    : (student.nativeLanguage || "Unknown");

  profile.push(`Student: ${student.name}`);
  profile.push(`Native language: ${nativeLang}`);
  ...
  if (student.dailyLanguage) {
    let dl = student.dailyLanguage;
    if (student.dailyLanguageOther) dl += ` (${student.dailyLanguageOther})`;
    profile.push(`Daily language environment: ${dl}`);
  }
  if (student.selfDescription) profile.push(`Student self-description: ${student.selfDescription}`);
  if (student.englishNeeds?.length) {
    let needs = student.englishNeeds.join(", ");
    if (student.englishNeedsOther) needs += `, ${student.englishNeedsOther}`;
    profile.push(`English needs: ${needs}`);
  }
  if (student.urgency) profile.push(`Urgency: ${student.urgency}`);
  if (student.usesEnglishAtWork === "Yes" && student.englishAtWorkHow?.length) {
    let how = student.englishAtWorkHow.join(", ");
    if (student.englishAtWorkHowOther) how += `, ${student.englishAtWorkHowOther}`;
    profile.push(`English at work: ${how}`);
  } else if (student.usesEnglishAtWork === "No") {
    profile.push(`English at work: No`);
  }
  if (student.examType) {
    const exam = student.examType + (student.examTypeOther ? ` (${student.examTypeOther})` : "");
    profile.push(`Exam: ${exam}`);
  }
  if (student.speaksTo?.length) {
    let sp = student.speaksTo.join(", ");
    if (student.speaksToOther) sp += `, ${student.speaksToOther}`;
    profile.push(`Speaks English with: ${sp}`);
  }
  if (student.communicationWhere?.length) profile.push(`Communication channels: ${student.communicationWhere.join(", ")}`);
  ...
  const obs = [];
  if (student.strengths) obs.push(`Strengths: ${student.strengths}`);
  if (student.troubleSpots) obs.push(`Trouble spots: ${student.troubleSpots}`);
  if (student.personality) obs.push(`Personality/rapport: ${student.personality}`);
  if (student.lessonPrefs?.length) {
    let lp = student.lessonPrefs.join(", ");
    if (student.lessonPrefsOther) lp += `, ${student.lessonPrefsOther}`;
    obs.push(`Lesson preferences: ${lp}`);
  }
...
<div class="doc-meta">${dateStr}<br>Student: ${student.name} | CEFR Level: ${student.level || "TBD"} | Native Language: ${nativeLang}</div>
...
<div class="section">
  ...
  <div class="vocab-box">
    <div class="vocab-item">
      ...
      <div class="vocab-l1">LanguageNameInThatLanguage: translation</div>
    </div>
  </div>
</div>
...
- Use the language's own name for itself: "Español: escalar", "Русский: повышать", "中文: 升级", "Türkçe: yükseltmek", "العربية: تصعيد", "Italiano: escalare", "Français: escalader", "Português: escalar", "Deutsch: eskalieren", "Polski: eskalować", "日本語: エスカレートする", "한국어: 확대하다", "Українська: ескалувати", etc.
- NEVER write "Native language:" — always use the language's own name.
...
- Internal consistency: does the question match the exercise type? If it says "correct the error," does the sentence actually contain the target error?
...
- Estimate realistic completion times based on task complexity and student's CEFR level.
