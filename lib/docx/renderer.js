...
import {
  ...
  BorderStyle, WidthType, ShadingType, VerticalAlign, ExternalHyperlink, UnderlineType,
} from "docx";
import { COLORS, FONT, SIZE, PAGE } from "./tokens.js";
import { SHEET_TIMEZONE } from "../dates.js";
import { quoteStudentWords } from "../format.js";

// ---------- small text helpers ----------

// Split a string into runs, honoring **bold** markers, [title](url) links, and
// bare URLs. Links become real clickable hyperlinks (teal, underlined).
function parseRuns(text, base = {}) {
  const s = String(text ?? "");
  const out = [];
  // [title](url)  |  **bold**  |  bare http(s) URL
  const re = /\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)|\*\*([^*]+)\*\*|(https?:\/\/[^\s)]+)/g;
  let last = 0, m;
  const link = (label, url) =>
    new ExternalHyperlink({
      link: url,
      children: [new TextRun({ ...base, text: label, color: COLORS.accent, underline: { type: UnderlineType.SINGLE } })],
    });
  while ((m = re.exec(s)) !== null) {
    if (m.index > last) out.push(new TextRun({ ...base, text: s.slice(last, m.index) }));
    if (m[1] && m[2]) out.push(link(m[1], m[2]));      // [title](url)
    else if (m[3]) out.push(new TextRun({ ...base, text: m[3], bold: true })); // **bold**
    else if (m[4]) out.push(link(m[4], m[4]));          // bare URL
    last = re.lastIndex;
  }
  if (last < s.length) out.push(new TextRun({ ...base, text: s.slice(last) }));
...
// Matches a [VERBATIM] … [/VERBATIM] block; the inner content must render
// exactly as given — no **bold**/link parsing, line breaks preserved.
const VERBATIM_RE = /\[VERBATIM\]\s*\n?([\s\S]*?)\n?\s*\[\/VERBATIM\]/gi;

// Normal flowing text: blank lines split paragraphs, single newlines join.
function flowParagraphs(body, opts = {}) {
  return String(body ?? "")
    .split(/\n{2,}/)
...
// Verbatim text: one paragraph per line, exactly as written — blank lines kept,
// markers like ** or [title](url) shown literally, nothing reflowed.
function verbatimParagraphs(text, opts = {}) {
  const { spacing, ...runOpts } = opts;
  return String(text ?? "")
    .replace(/\r\n?/g, "\n")
    .split("\n")
    .map(
      (line) =>
        new Paragraph({
          ...
          children: [new TextRun({ ...runOpts, text: line })],
        })
    );
}
...
function paragraphs(body, opts = {}) {
  ...
  while ((m = VERBATIM_RE.exec(src)) !== null) {
    if (m.index > last) out.push(...flowParagraphs(src.slice(last, m.index), opts));
    out.push(...verbatimParagraphs(m[1], opts));
    last = VERBATIM_RE.lastIndex;
  }
  if (last < src.length) out.push(...flowParagraphs(src.slice(last), opts));
  return out;
}
...
function renderCorrectionGrid(s, n) {
  ...
  // Two-rule system (§7/§8): "quotes" = the student's own words, bold = the
  // corrected target form. Applied here so every sheet is uniform regardless of
  // how the model marked the text.
  const rows = (s.rows || []).map(
    (r) =>
      new TableRow({
        children: [
          gridCell(quoteStudentWords(r.you), { color: COLORS.gridRed, italics: true }),
          gridCell(r.instead, { color: COLORS.gridGreen, bold: true }),
...
function renderCallout(s, n) {
  ...
  return n ? [sectionHeader(n, s.title || "Note"), box] : [box];
}
  ...
  (s.items || []).forEach((item) => {
    ...
    if (item.note) {
      out.push(calloutBox([new Paragraph({ spacing: { after: 0, line: 276 }, children: [
        new TextRun({ text: "Note:  ", bold: true, color: COLORS.amberBorder, size: SIZE.small }),
...
// Uses the sheet's own date when available (so the filename matches the header),
// and strips characters that aren't allowed in filenames.
export function sheetFilename(name, dateInput) {
  let date = null;
  if (dateInput) {
    const parsed = new Date(dateInput);
    if (!Number.isNaN(parsed.getTime())) date = parsed;
  }
  // A sheet date like "6 July 2026" parses to midnight in the server's own zone,
  // so format it there; only "no date at all" falls back to today in Mexico City.
  const tz = date ? {} : { timeZone: SHEET_TIMEZONE };
  const ref = date || new Date();
  const month = ref.toLocaleDateString("en-US", { month: "long", ...tz });
  const day = ref.toLocaleDateString("en-US", { day: "2-digit", ...tz });
  const safeName =
    String(name || "Student").trim().replace(/[\\/:*?"<>|]/g, "").trim() || "Student";
  return `Nick's Class ${month} ${day} - ${safeName}`;
}
