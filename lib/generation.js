...
GROUNDING — THE MOST IMPORTANT RULE. The CONTENT of the sheet (every correction, grammar point, vocabulary item, exercise, pronunciation item, topic, and link) comes ONLY from TODAY'S CLASS NOTES. The RECORDED HISTORY is for your awareness only — it tells you what's already been covered so you don't re-explain it — it is NOT a source of content for today's sheet.
- Do NOT introduce a topic, correction, vocab item, or pronunciation item that isn't in today's notes — not because it's a common error, not because the L1 guide mentions it, and not because it came up in a past lesson. If something is only in the history and not in today's notes, leave it out entirely.
- The methodology and the L1 guide are BACKGROUND REFERENCE, not a syllabus. The L1 guide exists only to help you explain WHY an error the student actually made happens, and to phrase L1-contrast notes for words/errors already in the notes. Never mine the L1 guide for topics to teach. If the notes say nothing about articles (a/an/the), do not add an articles section — even for a Slavic-language speaker.
- Do NOT narrate the relationship or invent continuity. Never write things like "we've fixed this before", "the job now is pure automaticity", "you asked for these again", "as we discussed", "you keep doing this", or reference past lessons, exams, deadlines, goals, or motivations unless that exact detail is in TODAY'S notes. When today's notes genuinely revisit an earlier point, at most one short neutral connecting phrase is okay — never manufacture shared history or personal context.
- When the notes are thin, make a shorter sheet. A focused sheet built only from the real lesson is far better than a padded one.
...
Keep the teacher out of it. The sheet is for the student and about their English — not about Nick or the class dynamic. Avoid teacher-centric lines like "I liked your observation" or "I was impressed by". A little encouragement is fine, but keep it brief and about the student's progress, not about your reactions or the relationship. Don't retell the class as a story about the two of you.
...
function historyText(history) {
  ...
  const lines = history
    ...
    .join("\n");
  return `For your awareness only — what earlier sheets covered, so you don't re-explain things already taught. Do NOT pull these topics, corrections, or vocab into today's sheet unless today's notes also raise them, and do NOT reference them as shared history. Today's content comes only from today's notes.\n\n${lines}`;
}
...
# RECORDED HISTORY (awareness only — NOT content for today; see the grounding rule)
${historyText(history)}

# TODAY'S CLASS NOTES (the ONLY source of content for this sheet)
${classNotes}

# TASK
Design today's review sheet using ONLY today's notes above for content. Today's date is ${dateStr}. Set student.date to that, and student.name and student.level from the profile. Choose the sections that fit this lesson and return the structured object.`;
}
