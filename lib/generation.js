...
import { todaySheetDate } from "./dates.js";
...
CORRECTION ACCURACY — HARD RULE. The correction grid's whole value is that it shows the student's REAL words; one fabricated quote and they can't trust any correction on the sheet. Build corrections ONLY from what the "Speaking corrections" notes actually contain. If the notes give a full sentence the student said, use that sentence. If they give only a phrase pair ("give a credit" → "make a down payment"), the row shows exactly that pair — do NOT reconstruct a full sentence around it, do NOT add context the student didn't provide, and never present any text as the student's own speech unless it arrived that way in the notes. The same applies anywhere on the sheet: never invent words and attribute them to the student.

VERBATIM PASS-THROUGH — HARD RULE. Anything in today's notes wrapped in [VERBATIM] … [/VERBATIM] must reach the sheet exactly as written: character for character, line breaks and list structure intact — never reworded, regenerated, summarized, shortened, corrected, or "improved". Copy the whole block, INCLUDING the [VERBATIM] and [/VERBATIM] markers, into the body or intro text of whichever section fits it best (use \n for the original line breaks); the app strips the markers and renders the content exactly as given. Don't restate or paraphrase the block's content anywhere else on the sheet.

The section types you can emit (choose freely, repeat or omit as needed):
- recap: { title, body }. A warm, plain second-person summary of what the class covered — lean on the "Lesson focus" note when one is given, lightly put in the sheet's voice: state what the class covered, concretely, and stop. Never list what the sheet contains (the student can see it), never open with a "Today we/was/ranged/ran…" template, no pep talk, no claims about future classes. Keep the body between 100 and 200 words.
- correction_grid: { title, intro?, rows:[{you, instead, why}] }. The workhorse: what the student said / what to say instead / why (tie the why to their L1). Lead with this when there are spoken errors. The app automatically wraps "you" in quotes and bolds "instead" — put plain text in those two fields, no added quotation marks or **asterisks**. In the "why" field, mark the teaching-point words so they don't vanish into the sentence, two ways: (a) word-as-word — when you refer to a word or phrase AS the item being taught ("X takes Y", "X needs Y before a noun", "X is the natural pronoun"), wrap it in bold AND quotation marks, like **"guide"** takes **"to"** for the destination; (b) word-in-use — when you show the target inside an example of correct usage (usually after a colon or "e.g."), bold only, no quotes: before a noun: **a lot of people**, **a lot of gold**. Mark ONLY the actual teaching-point words, never the whole sentence — but do NOT skip short function words ("to", "of", "them"); those are exactly the ones that get lost unmarked. Never alter marking inside a [VERBATIM] block.
...
Marking taught language — one system everywhere: **bold** the target language (vocabulary headwords, taught phrases and expressions, corrected forms) and put "quotes" around the student's own words whenever you cite them in body text. Never mark the same kind of item two different ways on one sheet. (Exception: correction_grid rows stay plain — the app styles those columns itself.)
...
export async function generateSheet({ student, classNotes, history = [] }) {
  ...
  // Ignore any history recorded for today — those are just iterations of today's
  // own lesson, not a prior class, so they must not be read back as "history".
  const todayStr = todaySheetDate();
  const priorHistory = (Array.isArray(history) ? history : []).filter((h) => (h?.date || "") !== todayStr);
  const messages = [{ role: "user", content: buildUser(student, classNotes, priorHistory) }];
