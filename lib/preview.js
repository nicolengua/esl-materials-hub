  ...
  switch (s.type) {
    ...
    case "correction_grid": {
      const rows = (s.rows || [])
        .map(
          (r) =>
            `<tr><td style="color:#B23A48;font-style:italic">${fmt(r.you)}</td><td style="color:#147A52">${fmt(r.instead)}</td><td>${fmt(r.why)}</td></tr>`
        )
        .join("");
      return (
        head +
        ...
        (s.items || [])
          .map(
            (it) =>
              ...
              (it.note ? `<div style="background:#FBEFD7;border:1px solid #E0A53B;padding:8px;border-radius:6px;font-size:13px"><strong>Heads-up:</strong> ${fmt(it.note)}</div>` : "") +
              `</div>`
          )
          .join("")
      );
