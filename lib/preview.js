...
// [VERBATIM] … [/VERBATIM] renders exactly as given: line breaks kept, no
// **bold**/link parsing — mirrors verbatimParagraphs in the Word renderer.
const paras = (body) => {
  const src = String(body ?? "");
  const re = /\[VERBATIM\]\s*\n?([\s\S]*?)\n?\s*\[\/VERBATIM\]/gi;
  let out = "", last = 0, m;
  while ((m = re.exec(src)) !== null) {
    out += flowParas(src.slice(last, m.index));
    out += `<p style="white-space:pre-wrap">${esc(m[1])}</p>`;
    last = re.lastIndex;
  }
  out += flowParas(src.slice(last));
  return out;
};
